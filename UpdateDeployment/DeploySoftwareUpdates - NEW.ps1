...
ForEach ($Detail in $DeploymentDetail)
{
    ...
    New-CMSoftwareUpdateDeployment -SoftwareUpdateGroupName $UpdateGroupName -CollectionName $TargetCollection -DeploymentName $DeploymentTitle -Description "Deployment Created by Powershell" -DeploymentType Required -VerbosityLevel AllMessages -TimeBasedOn LocalTime -AvailableDateTime $DeploymentDateTime -DeadlineDateTime $DeadlineDateTime -UserNotification DisplayAll -DownloadFromMicrosoftUpdate $True -UseMeteredNetwork $False -AcceptEula
